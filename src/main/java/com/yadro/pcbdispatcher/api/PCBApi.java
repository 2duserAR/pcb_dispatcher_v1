...
public interface PCBApi {
    ...
    @Operation(summary = "Получение списка всех плат")
    @ApiResponse(responseCode = "200", description = "Список плат успешно получен", 
                content = @Content(mediaType = "application/json", 
                array = @ArraySchema(schema = @Schema(implementation = PCB.class))))
    @GetMapping
    List<PCB> getAllPCBs();
} 